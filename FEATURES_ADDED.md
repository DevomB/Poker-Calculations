...
**Documentation:** [poker-calculations.devomb.com](https://poker-calculations.devomb.com) â€” API reference, examples, and guides (replaces repo `examples/*.mjs` scripts).

**Package metadata:** Node **18+**; entry `index.js` + `index.d.ts`; optional [`encode.js`](encode.js) for packed cards and PKST state. Prebuilt N-API binaries under `prebuilds/` (glibc + musl on Linux). No separate `poker-math.js` layer â€” all math is C++ via N-API.

**Async vs C++ parallelism:** `parallelHandSimulation` uses C++ `std::async` inside the native call. The `*Async` exports (`simulateHandOutcomeAsync`, etc.) use Nodeâ€™s libuv thread pool (`Napi::AsyncWorker`) so the **JavaScript event loop** stays responsive. Each `*Async` export accepts an optional trailing `{ signal?: AbortSignal }`; abort rejects with `AbortError` and stops CPU in cooperative hot loops (Monte Carlo, exact enumeration, benchmark, `decideAction` MC).
...
| Group | Export | Role |
| --- | --- | --- |
| **Hand resolution** | `evaluateBestHand(cards, options?)` | Best five of **1â€“7** cards; `HandEvalResult` (`rank`, `rankCategory`, `strength`, `kickers`) or `{ format: 'slim' }` â†’ `rankCategory` + `strength` only. |
...
| | `evaluateHandCategory(holeCards, board)` | Category label only (`highCard` â€¦ `royalFlush`); see [Hand rank labels](#hand-rank-labels). |
| | `validateCardString(card)` | `true` if `card` parses as one card (`Ah`, `10c`, â€¦). |
...
| | `canonicalCardString(card)` | Canonical two-character card (`Th`, `Ac`, â€¦); throws if invalid. |
| | `parseCompactCardList(text, options?)` | Parse compact card text; default `string[]`, `{ outFormat: 'packed' }` â†’ `Uint8Array` deck ids. |
| | `handRankCategoryOrder(category)` | Integer order `0..9` for `evaluateHandCategory` labels (`highCard` â€¦ `royalFlush`). |
...
| | `parallelHandSimulationAsync(â€¦, options?)` | Same as sync; Promise on libuv thread pool; optional `AbortSignal`. |
| | `simulateHandOutcomeBatch(specs[])` | Many MC spots â†’ `Float64Array` (`SimBatchSpec` per row). |
...
| | `exactHuEquityVsRandomHand(heroHoleCards, boardCards)` | Exact HU equity vs uniform random villain hand; board **empty (preflop) or 3â€“5** cards. |
| | `exactHuEquityVsRandomHandBatch(holes, boards, boardCards, out?)` | Packed batch exact HU vs random. |
| | `exactHuEquityVsRandomHandAsync(â€¦, options?)` | Same as sync; Promise on libuv thread pool; optional `AbortSignal`. |
| | `straightMadeFlopToRiverExactProbabilityAsync(â€¦, options?)` | Async sibling of exact flopâ†’river straight probability; optional `AbortSignal`. |
...
| | `decideActionAsync(â€¦, options?)` | Same as sync; Promise on libuv thread pool; optional `AbortSignal`. |
...
| | `breakevenCallEquityFromPotOddsDisplayRatio(displayPotToCallRatio)` | `1/(1+R)` from `potOddsRatioDisplay` ratio `R`; `0` when `R` is `+âˆž`. |
| | `potOddsDisplayRatioFromBreakevenCallEquity(breakevenEquity)` | Inverse: `(1âˆ’e)/e`; `+âˆž` when `e=0`; `0` when `e=1`. |
| | `formatPotOddsReducedFraction(potBeforeCall, toCall)` | Reduced integer ratio string `pot : to_call` (e.g. `100`,`50`â†’`2:1`); `toCall=0`â†’`âˆž:1`. |
| | `equityToWinningOddsAgainst(equity)` | Book-style `(1âˆ’p)/p`; `+âˆž` at `p=0`. |
| | `winningOddsAgainstToEquity(oddsAgainst)` | `1/(1+o)` for `oâ‰¥0`; `0` when `o=+âˆž`. |
| | `rakeFromPot(potChips, rakeFraction, rakeCap)` | Rake `min(fractionÃ—pot, cap)` for rake-adjusted helpers. |
...
| | `effectiveStack(...stacks)` | Minimum stack; empty â†’ `0`. |
...
| | `preflopCombosFromNotationsList(notations[])` | Sum of combo counts over a list; empty â†’ `0`. |
| **Heuristics** | `ruleOfFourEquity(outs)` | Out-count Ã— 4% cap heuristic (turn+river). |
| | `ruleOfTwoEquity(outs)` | Out-count Ã— 2% cap heuristic (one card). |
| | `estimatedOutsFromRuleOfTwo(equity, unseenCards)` | Uncapped inverse-style estimate `equityÃ—unseen/2` clamped to `[0, unseen]` (not exact inverse of capped rule-of-two). |
| | `estimatedOutsFromRuleOfFour(equity, unseenCards)` | Same for two streets: `equityÃ—unseen/4` clamped. |
| | `impliedBreakevenFutureWin(potBeforeCall, toCall, equity)` | Average extra future win needed for a neutral call; `+âˆž` if equity â‰¤ 0. |
...
| | `flopToRiverAtLeastOneHitUnionThreeCategories(...)` | Three categories; inclusionâ€“exclusion on union size, then same two-street formula. |
| | `flopToRiverAtLeastOneHitUnionFourCategories(...)` | Four categories; full inclusionâ€“exclusion on card-count intersections, then same two-street formula. |
...
| | `agrestiCoullInterval(successes, nTrials, z)` | Agrestiâ€“Coull interval (same `{ lower, upper }` shape). |
| | `normalWaldBinomialInterval(successes, nTrials, z)` | Normal (Wald) `pÌ‚ Â± zÂ·SE` clamped to `[0,1]` (weak near 0/1 with small `n`). |
| | `monteCarloTrialsForHoeffdingBound(epsilon, delta)` | Hoeffding: smallest `n` with `n â‰¥ ln(2/Î´)/(2ÎµÂ²)` for uniform MC error bound. |
| | `riskOfRuinDiffusionApprox(driftPerHand, variancePerHand, bankroll)` | `exp(âˆ’2Î¼B/ÏƒÂ²)` style ROR; returns `1` if drift â‰¤ 0. |
...
| **Kelly & jam toys** | `kellyCriterionBinary(winProbability, netOdds)` | Full Kelly `(pÂ·b âˆ’ (1âˆ’p)) / b` for net odds `b`. |
| | `chubukovSymmetricJamBreakevenStack(deadMoneyChips, equity)` | Toy symmetric jam: `S = equityÂ·dead/(1âˆ’2Â·equity)` for `equity < 0.5`; `+âˆž` if `equity > 0.5`. |
...
| | `chubukovMaxSymmetricJamStackBinarySearch(heroHoleCards[], boardCards[], deadMoneyChips, maxStackChips)` | Same search on equity from `exactHuEquityVsRandomHand` (board 3â€“5); `maxStackChips` clamped like native `double` â†’ int cap. |
...
| | `bluffToValueRatio(potBeforeBet, betSize)` | Polarized river combo ratio `bet / (pot + 2Ã—bet)`. |
...
| | `breakevenFoldEquitySemiBluff(potBeforeHeroBet, heroBetSize, equityWhenCalled, totalPotIfCalled)` | Two-outcome model; may exceed 1 if line is âˆ’EV even if villain always folds. |
...
| | `icmHarvillePlacementProbabilities(stacks[])` | Full `nÃ—n` Harville placement matrix (per player, per finish rank). |
...
| Rule | Detail |
| --- | --- |
| Ranks | `2`â€“`9`, `J`, `Q`, `K`, `A`; ten as `T` or `10` (canonical output uses `T`) |
...
Returned by `evaluateBestHand` â†’ `rank` and `evaluateHandCategory`. `handRankCategoryOrder` maps name â†’ `0..9`.
...
`evaluateBestHand` â†’ `kickers`: length-5 array of encoded kicker values (internal rank ordering for tie-breaks).
...
| **`WilsonScoreInterval`** | `lower`, `upper` (same shape for Agrestiâ€“Coull and Wald intervals) |
...
**Hero resolution:** `heroSeat` if passed; else acting playerâ€™s seat; else first playerâ€™s seat.
...
| Exact equity | [`include/poker/exact_equity.hpp`](include/poker/exact_equity.hpp) | Enumeration equity vs random hand; exact flopâ†’river straight-or-better; Chubukov max integer jam stack from hand. |
...
| **State & rules** | `PokerGameState`, blinds, pot, per-street commits, phase machine (pre-flop â†’ river â†’ showdown), `GameEngine::apply_action` with `Decision`. |
